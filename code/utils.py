...
def get_split(x, y, num_clinical):
    x_train, x_test, y_train, y_test = train_test_split(x, y, test_size=0.2, stratify=y[:, num_clinical + 1])
    x_train, x_vali, y_train, y_vali = train_test_split(x_train, y_train, test_size=len(y_test), stratify=y_train[:, num_clinical + 1])
